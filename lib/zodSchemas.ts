...
export const onboardingSchemaLocale = toTypedSchema(z.object({
  username: z
    .string()
    .min(3)
    .max(150)
...
export const aboutSettingsSchema = toTypedSchema(z.object({
  fullName: z.string().min(3).max(150),
  profileImage: z.string(),
}));

export const eventTypeSchema = toTypedSchema(z.object({
...
export const updateEventTypeSchema = toTypedSchema(z.object({
  title: z.string().min(3).max(150),
  duration: z.string().min(1).max(100),
  url: z.string().min(3).max(150),
  description: z.string().min(3).max(300),
...
export const createMeetingTypeSchema = toTypedSchema(z.object({
  email: z.string().min(1, { message: 'Email is required' }).email({ message: 'Must be a valid email' }),
  name: z.string().min(5, { message: 'Name must be at least 5 characters' }).max(30, { message: 'Name must have maximum 30 characters' }),
}));
